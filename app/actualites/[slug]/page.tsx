...
function RenderBlock({
  ...
  switch (block.type) {
      ...
      if (block.level === 3) {
        return (
          <h3 style={{
            ...
            fontSize: "clamp(1.05rem, 1.8vw, 1.3rem)", fontWeight: 700,
            letterSpacing: "-0.02em", lineHeight: 1.25,
            margin: "2rem 0 0.85rem", color: cs.dark,
          }}>
            {block.text}
          </h3>
        );
      }
      return <h2 className="sl-h2" style={{ color: cs.dark }}>{block.text}</h2>;
    ...
    case "video":
      return (
        <div style={{ margin: "2.2rem 0", borderRadius: 16, overflow: "hidden",
    ...
    case "factbox":
      return (
        <div className="sl-keyfacts" style={{ borderTopColor: cs.color, background: cs.bg }}>
        ...
        <div style={{ margin: "2rem 0" }}>
          {block.title && (
            <div style={{ fontSize: "0.6rem", fontWeight: 800,
              ...
              color: cs.color, marginBottom: "1rem" }}>
              {block.title}
            </div>
          )}
            ...
            {block.items.map((item, i) => (
              <div key={i} style={{ display: "flex", gap: "0.85rem",
                ...
                <div style={{ width: 22, height: 22, borderRadius: "50%", flexShrink: 0,
                  background: cs.bg, display: "flex", alignItems: "center",
                  justifyContent: "center" }}>
                  <svg width="11" height="11" viewBox="0 0 24 24" fill="none"
                    stroke={cs.color} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M20 6L9 17l-5-5"/>
                  </svg>
                </div>
                <div>
                  <div style={{ fontSize: "0.88rem", fontWeight: 600,
                    color: "#141410", lineHeight: 1.35 }}>{item.label}</div>
                  {item.detail && (
                    <div style={{ fontSize: "0.75rem", color: "#928E80",
                      marginTop: "0.2rem", lineHeight: 1.5 }}>{item.detail}</div>
                  )}
                </div>
              </div>
            ))}
      ...
      return (
        <div style={{ margin: "2rem 0" }}>
          {block.title && (
            <div style={{ fontSize: "0.6rem", fontWeight: 800,
              letterSpacing: "0.15em", textTransform: "uppercase",
              color: cs.color, marginBottom: "1.25rem" }}>
              {block.title}
            </div>
          )}
          <div style={{ display: "flex", flexDirection: "column", gap: "0" }}>
            {block.items.map((step, i) => (
              <div key={i} style={{ display: "flex", gap: "1rem",
                ...
                <div style={{ flex: 1, paddingTop: "0.5rem", paddingBottom: "1.5rem" }}>
                  <div style={{ fontSize: "0.95rem", fontWeight: 700,
                    color: "#141410", marginBottom: "0.3rem" }}>{step.label}</div>
                  <div style={{ fontSize: "0.82rem", color: "#928E80", lineHeight: 1.65 }}>
                    {step.desc}
                  </div>
                </div>
              </div>
          ...
          {block.items.map((b, i) => (
            <div key={i} style={{
              ...
              <div style={{ fontSize: "0.58rem", fontWeight: 800,
                ...
                marginBottom: "0.25rem" }}>{b.label}</div>
              <div style={{ fontSize: "0.85rem", fontWeight: 600,
                color: "#141410", lineHeight: 1.35 }}>{b.value}</div>
            </div>
          ))}
        </div>
      );
      ...
      return (
        <div style={{ margin: "2rem 0", overflowX: "auto" }}>
          {block.title && (
            <div style={{ fontSize: "0.6rem", fontWeight: 800,
              letterSpacing: "0.15em", textTransform: "uppercase",
              color: cs.color, marginBottom: "1rem" }}>{block.title}</div>
          )}
          <table style={{ width: "100%", borderCollapse: "collapse",
            ...
            <tbody>
              {block.rows.map((row, ri) => (
                <tr key={ri} style={{ borderTop: "1px solid rgba(20,20,16,.06)" }}>
                  <td style={{ padding: "0.75rem 1rem", fontWeight: 600,
                    color: "#141410", fontSize: "0.82rem" }}>{row.label}</td>
                  {row.values.map((val, vi) => (
                    <td key={vi} style={{ padding: "0.75rem 1rem",
                      color: "#38382E", fontSize: "0.82rem" }}>{val}</td>
                  ))}
                </tr>
              ))}
            </tbody>
    ...
    case "alert": {
      ...
      const alertIcons: Record<string, string> = {
        info: "ℹ️", warning: "⚠️", tip: "💡",
      };
      const v = block.variant ?? "info";
      return (
        <div style={{ margin: "1.8rem 0", padding: "1.1rem 1.4rem",
          borderLeft: `4px solid ${alertColors[v]}`,
          background: `${alertColors[v]}10`,
          borderRadius: "0 12px 12px 0",
          display: "flex", gap: "0.75rem", alignItems: "flex-start" }}>
          <span style={{ fontSize: "1rem", flexShrink: 0 }}>{alertIcons[v]}</span>
          <p style={{ fontSize: "0.88rem", color: "#38382E", lineHeight: 1.7, margin: 0 }}>
            {block.message}
          </p>
        </div>
      );
      ...
      return (
        <a href={block.url} target="_blank" rel="noopener noreferrer"
          style={{ display: "flex", alignItems: "center", gap: "0.9rem",
            ...
            textDecoration: "none", color: "#141410",
            boxShadow: "0 2px 8px rgba(20,20,16,.05)" }}>
          {block.favicon && (
            block.favicon.startsWith("http") || block.favicon.startsWith("/")
              ? <img src={block.favicon} alt="" style={{ width: 20, height: 20, borderRadius: 4 }} />
          ...
          <span style={{ fontSize: "0.72rem", color: "#928E80", flexShrink: 0 }}>↗</span>
        </a>
      );
    ...
    case "related": {
      ...
      return (
        <Link href={`/actualites/${block.slug}`}
          ...
          <span style={{ fontSize: "0.58rem", fontWeight: 800,
            letterSpacing: "0.12em", textTransform: "uppercase",
            color: cs.color, flexShrink: 0 }}>À lire aussi</span>
    ...
    case "download":
      return (
        <a href={block.url} download
          ...
          ↓ {block.label}
          {block.size && (
            <span style={{ opacity: 0.6, fontWeight: 400 }}>({block.size})</span>
          )}
        </a>
      );
    ...
    case "apply":
      return (
        <div style={{ margin: "2rem 0", padding: "1.5rem 1.75rem",
          background: "#141410", borderRadius: 20,
          border: "1px solid rgba(248,246,241,.06)" }}>
          {block.deadline && (
            <div style={{ fontSize: "0.58rem", fontWeight: 800,
              letterSpacing: "0.15em", textTransform: "uppercase",
              color: "rgba(248,246,241,.35)", marginBottom: "0.35rem" }}>
              Date limite : {block.deadline}
            </div>
          )}
          {block.note && (
            <p style={{ fontSize: "0.82rem", color: "rgba(248,246,241,.55)",
              lineHeight: 1.65, marginBottom: "1rem" }}>{block.note}</p>
          )}
          <a href={block.url} target="_blank" rel="noopener noreferrer"
            style={{ display: "inline-flex", alignItems: "center", gap: "0.5rem",
              background: "#C08435", color: "#fff", padding: "0.85rem 1.75rem",
              borderRadius: 100, fontWeight: 800, fontSize: "0.88rem",
              textDecoration: "none", boxShadow: "0 6px 20px rgba(192,132,53,.4)" }}>
            {block.label} ↗
          </a>
        </div>
      );
      ...
      return (
        <div style={{ margin: "2rem 0" }}>
          ...
          <div style={{ background: "#fff", borderRadius: 16,
            border: "1px solid rgba(20,20,16,.08)", overflow: "hidden" }}>
            {block.sessions.map((s, i) => (
              <div key={i} style={{ display: "flex", gap: "1rem",
                alignItems: "flex-start", padding: "0.9rem 1.25rem",
                borderBottom: i < block.sessions.length - 1
                  ? "1px solid rgba(20,20,16,.06)" : "none" }}>
                <div style={{ fontSize: "0.72rem", fontWeight: 800,
                  color: cs.color, flexShrink: 0, minWidth: 52,
                  fontFamily: "'Fraunces', Georgia, serif" }}>{s.time}</div>
                <div>
                  <div style={{ fontSize: "0.88rem", fontWeight: 600,
                    color: "#141410", lineHeight: 1.3 }}>{s.title}</div>
                  {s.speaker && (
                    <div style={{ fontSize: "0.72rem", color: "#928E80",
                      marginTop: "0.2rem" }}>{s.speaker}</div>
                  )}
                </div>
              </div>
            ))}
        ...
        <div style={{ margin: "2rem 0" }}>
          ...
          <div style={{ display: "grid",
            ...
            {block.people.map((p, i) => (
              <div key={i} style={{ padding: "1.1rem", borderRadius: 14,
                background: "#fff", border: "1px solid rgba(20,20,16,.08)",
                ...
                <div>
                  <div style={{ fontSize: "0.85rem", fontWeight: 700, color: "#141410" }}>
                    {p.name}
                  </div>
                  <div style={{ fontSize: "0.7rem", color: cs.color, fontWeight: 600 }}>
                    {p.role}
                  </div>
                  {p.org && (
                    <div style={{ fontSize: "0.65rem", color: "#928E80", marginTop: "0.1rem" }}>
                      {p.org}
                    </div>
                  )}
                </div>
    ...
    case "location":
      return (
        <div style={{ margin: "2rem 0", padding: "1.1rem 1.4rem",
          background: "#fff", borderRadius: 14,
          border: "1px solid rgba(20,20,16,.08)",
          boxShadow: "0 1px 4px rgba(20,20,16,.04)" }}>
          <div style={{ display: "flex", gap: "0.75rem", alignItems: "flex-start" }}>
            ...
            <div>
              <div style={{ fontSize: "0.88rem", fontWeight: 700, color: "#141410" }}>
                {block.label}
              </div>
              {block.address && (
                <div style={{ fontSize: "0.75rem", color: "#928E80", marginTop: "0.2rem" }}>
                  {block.address}
                </div>
              )}
            </div>
            {block.mapUrl && (
              <a href={block.mapUrl} target="_blank" rel="noopener noreferrer"
                style={{ marginLeft: "auto", fontSize: "0.72rem", fontWeight: 700,
                  color: cs.color, textDecoration: "none", flexShrink: 0 }}>
                Voir sur la carte ↗
              </a>
            )}
          </div>
          {block.mapUrl && (
            <div style={{ marginTop: "0.85rem", borderRadius: 10, overflow: "hidden",
              height: 200, background: "#F0EDE4" }}>
              <iframe src={block.mapUrl} width="100%" height="200"
...
(sb as any).rpc("increment_article_views", { article_id: data.id })
  .then(({ error: rpcErr }: any) => {
    ...
    const { data: rel } = await sb
      .from("articles")
      .select("id,slug,title,excerpt,category,author_name,reading_time,cover_url,image_gradient,created_at")
      .eq("published", true)
      .neq("id", data.id)
  ...
  return (
    <>
      ...
      <div style={{ position: "fixed", top: 0, left: 0, right: 0, height: 3,
        zIndex: 9999, background: "rgba(20,20,16,.1)" }}>
      ...
      {isPreview && !article.published && (
        <div style={{ position: "fixed", top: 3, left: 0, right: 0, zIndex: 9998,
          background: "#C08435", color: "#fff", textAlign: "center",
          ...
          <Link href={`/admin/actualites/${article.id}`}
            style={{ marginLeft: "1.5rem", color: "#fff", textDecoration: "underline",
              fontSize: "0.72rem" }}>
            ← Retour à l&apos;éditeur
          </Link>
        </div>
      ...
      <main style={{ paddingTop: isPreview && !article.published ? 34 : 0 }}>
        ...
        <section style={{ position: "relative", overflow: "hidden",
          ...
          <div style={{ position: "absolute", inset: 0,
            background: "linear-gradient(180deg,rgba(0,0,0,.2) 0%,transparent 28%,rgba(0,0,0,.55) 65%,rgba(0,0,0,.94) 100%)" }}/>
          <div style={{ position: "absolute", inset: 0,
            background: "linear-gradient(105deg,rgba(0,0,0,.5) 0%,transparent 52%)" }}/>
          ...
          <div style={{ position: "absolute", inset: 0,
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "0.75rem",
              ...
              <Link href="/actualites" style={{ display: "flex", alignItems: "center",
                ...
                color: "rgba(255,255,255,.48)", textDecoration: "none" }}
            ...
            <div className="sl-layout">
              ...
              <article className="sl-main">
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "0.65rem",
                  ...
                  <button className="sl-btn" onClick={() => navigator.share?.({ title: article.title, url: window.location.href })}>
                    <IcoShare /> Partager
                  </button>
                  <div style={{ marginLeft: "auto", display: "flex",
                    alignItems: "center", gap: "0.55rem" }}>
                    <div style={{ width: 72, height: 4,
                      background: "rgba(20,20,16,.1)", borderRadius: 100, overflow: "hidden" }}>
                      <div style={{ height: "100%", background: cs.color,
                        borderRadius: 100, width: `${progress}%`, transition: "width .18s" }}/>
                    </div>
                ...
                <div style={{ paddingTop: "2.5rem",
                  borderTop: "2px solid rgba(20,20,16,.08)", paddingBottom: "3rem" }}>
              ...
              <aside className="sl-sidebar">
                ...
                <div className="sl-card">
                  ...
                  <div style={{ height: 6, background: "rgba(20,20,16,.09)",
                    ...
                    <div style={{ height: "100%", borderRadius: 100,
                      background: cs.color, width: `${progress}%`,
                ...
                {headings.length > 0 && (
                  <div className="sl-card" style={{ padding: 0 }}>
                    <div style={{ padding: "1.2rem 1.2rem 0.8rem",
                      ...
                      <div className="sl-card-label" style={{ marginBottom: 0 }}>
                        Sommaire
                      </div>
                    </div>
                    {headings.map((b, si) => (
                      <div key={si} style={{ display: "flex", gap: "0.8rem",
                        ...
                        borderBottom: si < headings.length - 1
                          ? "1px solid rgba(20,20,16,.06)" : "none" }}>
                      ...
                      return (
                        <Link key={art.id} href={`/actualites/${art.slug}`}
                          ...
                          <div style={{ display: "flex", gap: "0.8rem",
                            ...
                            borderBottom: ri < relArticles.length - 1
                              ? "1px solid rgba(20,20,16,.06)" : "none",
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <div style={{ fontSize: "0.54rem", fontWeight: 800,
                                letterSpacing: "0.1em", textTransform: "uppercase",
                                color: acs.color, marginBottom: "0.28rem" }}>
                                {art.category}
                              </div>
                              <div style={{ fontFamily: "'Fraunces', Georgia, serif",
                                fontSize: "0.8rem", fontWeight: 700, color: "#141410",
                                lineHeight: 1.28,
                              ...
                              <div style={{ fontSize: "0.57rem", color: "#928E80",
                                marginTop: "0.28rem" }}>
                ...
                {/* Newsletter */}
                <div className="sl-card"
                  style={{ background: "#141410", border: "none", padding: "1.5rem" }}>
                  <div style={{ fontSize: "0.58rem", fontWeight: 800,
                  ...
                  <Link href="/newsletter" style={{ display: "block",
                    textAlign: "center", background: "#C08435", color: "#fff",
                    borderRadius: 100, padding: "0.65rem",
                      ...
                      <Link href={`/actualites/${art.slug}`}
                        ...
                        <div className="sl-related-card">
                          <div style={{ height: 215, position: "relative",
                            ...
                            <span style={{ position: "absolute", top: "0.9rem",
                              left: "0.9rem", fontSize: "0.55rem", fontWeight: 800,
                              letterSpacing: "0.1em", textTransform: "uppercase",
                            ...
                            <div style={{ position: "absolute", bottom: 0,
                              left: 0, right: 0, height: 3, background: acs.color }}/>
                          </div>
                          <div style={{ padding: "1.35rem 1.5rem",
                            ...
                            <div style={{ display: "flex", alignItems: "center",
                              ...
                              <span style={{ fontSize: "0.6rem", color: "#928E80" }}>
                                <b style={{ color: "#38382E" }}>{art.author_name}</b>
                              </span>
