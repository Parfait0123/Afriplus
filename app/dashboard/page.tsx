...
export default function DashboardPage() {
  ...
  const saveProfile = async () => {
    ...
    if (!profile) {
      setSaveError("Profil non chargé, rechargez la page.");
      return;
    }
    if (!profile.id) {
      setSaveError("ID profil manquant.");
      return;
    }
  ...
  const changePassword = async () => {
    ...
    setPwdSaving(false);
    if (updateErr) {
      setPwdError("Erreur : " + updateErr.message);
      return;
    }
    setPwdOk(true);
  ...
  const uploadAvatar = async (file: File) => {
    ...
    if (storageError) {
      ...
      setSaveError(
        "Upload échoué. Vérifiez que le bucket 'avatars' existe et est public.",
      );
    ...
    <>
      ...
      <div className="db-page">
          ...
          <div className="db-header-inner">
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div className="db-header-sub">
                {[profile?.domain, profile?.country]
                  .filter(Boolean)
                  .join(" · ") || "Complétez votre profil"}
              </div>
            </div>
            ...
            {TABS.map((t) => {
              ...
              return (
                <button
                  ...
                  style={{
                    borderBottom: `2.5px solid ${active ? "#C08435" : "transparent"}`,
                    color: active ? "#141410" : "#928E80",
                ...
                >
                  ...
                  {count !== null && count > 0 && (
                    <span
                      className="db-tab-badge"
                      style={{
                        background: active ? "#C08435" : "#EAE7DE",
        ...
        <main className="db-body">
              ...
              <ChipsRow
                items={[
                  { k: "all", l: "Tout", n: saves.length },
                  {
                    k: "article",
                    l: "Articles",
                    n: saves.filter((s) => s.content_type === "article").length,
                  },
                  {
                    k: "scholarship",
                    l: "Bourses",
                    n: saves.filter((s) => s.content_type === "scholarship")
                      .length,
                  },
                  {
                    k: "opportunity",
                    l: "Opportunités",
                    n: saves.filter((s) => s.content_type === "opportunity")
                      .length,
                  },
                  {
                    k: "event",
                    l: "Événements",
                    n: saves.filter((s) => s.content_type === "event").length,
                  },
                ].filter((f) => f.k === "all" || f.n > 0)}
                active={saveFilter}
                onChange={setSaveFilter}
              />
              {filteredSaves.length === 0 ? (
                <EmptyState
                  icon="☆"
                  title="Aucune sauvegarde"
                  desc="Appuyez sur ☆ sur n'importe quelle fiche pour la retrouver ici."
                />
              ) : (
            ...
            <section style={{ animation: "db-rise .3s ease both" }}>
                ...
                items={[
                  ...
                  ...(Object.keys(STATUS_LABEL) as ApplicationStatus[])
                    ...
                    .map((s) => ({
                      k: s,
                      l: STATUS_LABEL[s],
                      n: applications.filter((a) => a.status === s).length,
              ...
              {filteredApps.length === 0 ? (
                <EmptyState
                  ...
                  desc="Suivez vos dossiers depuis les fiches bourses et opportunités."
                />
              ) : (
                <div
                  style={{
                      ...
                      >
                        <div className="db-app-row">
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            >
                              <Tag>
                                {app.content_type === "scholarship"
                                  ? "Bourse"
                                  : "Opportunité"}
                              </Tag>
                              {urgent && (
                                <Tag color="#B8341E" bg="#FAEBE8">
                                  ⏳ {daysLeft}j restants
                                </Tag>
                              )}
                            ...
                            <Link
                              href={`${CONTENT_PATHS[app.content_type]}/${app.content_slug}`}
                              style={{
                                display: "inline-flex",
                                alignItems: "center",
                                gap: "0.25rem",
                          ...
                          <div className="db-app-actions">
                            <select
                              ...
                              style={{
                                ...
                                borderRadius: 10,
                                border: `1.5px solid ${sc.color}45`,
                                background: sc.bg,
                                color: sc.color,
                                fontWeight: 700,
                                fontSize: "0.72rem",
            ...
            <section style={{ animation: "db-rise .3s ease both" }}>
              {events.length === 0 ? (
                <EmptyState
                  ...
                  desc="Inscrivez-vous à des événements pour les retrouver ici avec export .ics."
                />
              ) : (
                <div className="db-events-grid">
                  {events.map((r, i) => {
                    const isPast =
                      !!r.event_date && new Date(r.event_date) < new Date();
                    const dateObj = r.event_date
                      ? new Date(r.event_date)
                      : null;
                    return (
                      <div
                        key={r.id}
                      ...
                      >
                        <div
                          style={{
                            background: isPast
                              ? "#EAE7DE"
                              : "linear-gradient(135deg,#FBF4E8 0%,#F5EDD8 100%)",
                            borderBottom: "1px solid rgba(192,132,53,.1)",
          ...
          {tab === "profile" && (
            <section
              className="db-profile-wrap"
              style={{ animation: "db-rise .3s ease both" }}
            >
              {/* ── Carte identité hero ── */}
              <div
                style={{
                  background:
                    "linear-gradient(135deg,#141410 0%,#1e1d17 60%,#2a2618 100%)",
                  borderRadius: 24,
              ...
              >
                ...
                <div
                  style={{
                    ...
                    height: 2,
                    background:
                      "linear-gradient(90deg,transparent,#C08435 30%,#E8B86D 60%,transparent)",
                  }}
                />
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    >
                      {[profile?.domain, profile?.country, profile?.city]
                        .filter(Boolean)
                    ...
                    {profile?.id && (
                      <Link
                        ...
                        style={{
                          ...
                          color: "#C08435",
                          textDecoration: "none",
                          border: "1px solid rgba(192,132,53,.3)",
                          padding: "0.25rem 0.7rem",
                          borderRadius: 100,
                          background: "rgba(192,132,53,.08)",
                        }}
                      >
              ...
              >
                ...
                >
                  <div
                    style={{
                      ...
                      height: 32,
                      borderRadius: 10,
                      background: "linear-gradient(135deg,#C08435,#8C5A1C)",
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                    }}
                ...
                <FormSection title="">
                  ...
                  <Field label="Bio courte">
                    <textarea
                      rows={3}
                      placeholder="Quelques mots sur vous, vos ambitions…"
                      className="db-inp"
                    ...
                    [
                      {
                        k: "notify_saves",
                        l: "Rappels deadlines",
                        d: "Email avant l'expiration de vos sauvegardes",
                      },
                      {
                        k: "notify_news",
                        l: "Newsletter hebdomadaire",
                        d: "Le récap AroMe chaque semaine",
                      },
                    ] as const
                  ).map((pref) => {
                    const on = !!(profileForm as Record<string, unknown>)[
                      pref.k
                    ];
                    return (
                      <div
                        ...
                        style={{
                          ...
                          background: "#F8F6F1",
                          marginBottom: "0.6rem",
                          border: "1px solid rgba(20,20,16,.05)",
                        }}
                      >
                        <div style={{ flex: 1 }}>
                          <div
                ...
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 14,
                    background: savedOk
                      ? "linear-gradient(135deg,#1A5C40,#0F3828)"
                      : saving
                        ? "rgba(192,132,53,.6)"
                        : "linear-gradient(135deg,#C08435,#8C5A1C)",
                    color: "#fff",
                    border: "none",
                    fontWeight: 700,
                    fontSize: "0.92rem",
                    cursor:
                      saving || avatarUploading ? "not-allowed" : "pointer",
                    transition: "all .2s",
                    fontFamily: "inherit",
                    display: "flex",
                    alignItems: "center",
              ...
              <div className="db-card" style={{ padding: "1.75rem" }}>
                ...
                >
                  <div
                    style={{
                      ...
                      height: 32,
                      borderRadius: 10,
                      background: "linear-gradient(135deg,#1A5C40,#0F3828)",
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                    }}
                ...
                {[
                  {
                    k: "current" as const,
                    l: "Mot de passe actuel",
                    ph: "Votre mot de passe actuel",
                  },
                  {
                    k: "next" as const,
                    l: "Nouveau mot de passe",
                    ph: "8 caractères minimum",
                  },
                  {
                    k: "confirm" as const,
                    l: "Confirmer le nouveau",
                    ph: "Répétez le nouveau mot de passe",
                  },
                ].map((f) => (
                  <Field key={f.k} label={f.l}>
                    <div style={{ position: "relative" }}>
                      <input
                        ...
                        value={pwdForm[f.k]}
                        onChange={(e) =>
                          setPwdForm((p) => ({ ...p, [f.k]: e.target.value }))
                        }
                        onKeyDown={(e) => {
                ...
                {pwdForm.next.length > 0 && (
                  <div style={{ marginBottom: "1rem", marginTop: "-0.5rem" }}>
                    ...
                    >
                      {[1, 2, 3, 4].map((i) => {
                        const score =
                          pwdForm.next.length >= 12 &&
                          /[A-Z]/.test(pwdForm.next) &&
                          /[0-9]/.test(pwdForm.next) &&
                          /[^A-Za-z0-9]/.test(pwdForm.next)
                            ? 4
                            : pwdForm.next.length >= 10 &&
                                /[A-Z]/.test(pwdForm.next) &&
                                /[0-9]/.test(pwdForm.next)
                              ? 3
                ...
                >
                  {pwdSaving ? (
                    ...
                    <>
                      <IcoCheck /> Mot de passe mis à jour !
                    </>
                  ) : (
                    "Changer le mot de passe"
                  )}
              ...
              >
                ...
                >
                  <div
                    style={{
                      ...
                      height: 32,
                      borderRadius: 10,
                      background: "linear-gradient(135deg,#B8341E,#8A2A1A)",
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                    }}
                ...
                <button
                  ...
                  style={{
                    ...
                    justifyContent: "center",
                    gap: "0.5rem",
                    boxShadow: "0 4px 12px rgba(184,52,30,.2)",
                  }}
                >
                  <svg
                    width="16"
      ...
      <nav className="db-bnav">
        {TABS.map((t) => {
          ...
          return (
            <button
              key={t.key}
              className="db-bnav-btn"
              onClick={() => setTab(t.key)}
              style={{ color: active ? "#C08435" : "rgba(255,255,255,.4)" }}
            >
              <div
                className="db-bnav-icon"
                style={{
                  background: active ? "rgba(192,132,53,.15)" : "transparent",
                }}
              >
                <t.Icon />
                {count !== null && count > 0 && (
                  <span className="db-bnav-dot">
                    {count > 9 ? "9+" : count}
                  </span>
                )}
              </div>
              <span
                className="db-bnav-label"
                style={{
                  color: active ? "#C08435" : "rgba(255,255,255,.35)",
                  fontWeight: active ? 700 : 500,
                }}
              >
                {t.label}
    ...
    <label style={{ position: "relative", cursor: "pointer", flexShrink: 0 }}>
      <div
        style={{
          ...
          height: 68,
          borderRadius: "50%",
          background: "linear-gradient(135deg,#C08435,#7a5220)",
          display: "flex",
          alignItems: "center",
          justifyContent: "center",
          fontFamily: "'Fraunces',serif",
          ...
          color: "#fff",
          overflow: "hidden",
          boxShadow: "0 4px 18px rgba(192,132,53,.25)",
          border: "3px solid rgba(192,132,53,.3)",
        }}
      >
        {uploading ? (
          <div
      ...
      {items.map((f) => {
        ...
        return (
          <button
            ...
            style={{
              background: isActive ? (f.bg ?? "#FBF4E8") : "#fff",
              color: isActive ? (f.color ?? "#C08435") : "#928E80",
...
function SaveCard({
  ...
  const label =
    CONTENT_LABEL[save.content_type as keyof typeof CONTENT_LABEL] ??
    save.content_type;
  return (
    <div
      className="db-card-sm"
    ...
    >
      ...
      {(meta?.deadline as string) && (
        <div style={{ fontSize: "0.68rem", color: "#B8341E", fontWeight: 700 }}>
          ⏳ Deadline : {String(meta?.deadline)}
        </div>
      )}
      <Link
        href={`${CONTENT_PATHS[save.content_type]}/${save.content_slug}`}
        style={{
          display: "inline-flex",
          alignItems: "center",
          gap: "0.3rem",
  ...
  return (
    <button
      ...
      style={{
        ...
        flexShrink: 0,
        position: "relative",
        background: on ? "#1A5C40" : "rgba(20,20,16,.12)",
...
function IcoICS() {
  return (
    ...
    >
      ...
      <line x1="8" y1="2" x2="8" y2="6" />
      <line x1="3" y1="10" x2="21" y2="10" />
      <polyline points="12 14 12 18 16 16" />
    </svg>
  );
}
function IcoCheck() {
