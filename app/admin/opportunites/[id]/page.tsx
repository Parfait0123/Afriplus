...
export default function OpportunityEditorPage({
  ...
  const set = useCallback(
    <K extends keyof OpportunityState>(k: K, v: OpportunityState[K]) =>
    ...
    autoSaveRef.current = setTimeout(() => {
      ...
      try {
        localStorage.setItem(
          `afripulse_draft_opportunity_${itemIdRef.current}`,
  ...
  const getDefaultMetaTitle = () => {
    if (state.title)
      return `${state.title} — ${state.company || "Offre d'emploi"} | AroMe`;
    return "Offre d'emploi | AroMe";
  };
  const getDefaultMetaDesc = () => {
    const firstText = extractFirstTextFromBlocks(state.blocks);
    if (firstText && firstText.length > 0) {
      return firstText.length > 160
        ? firstText.slice(0, 157) + "..."
        : firstText;
    }
  ...
  const fillDefaultMetaDesc = () => {
    if (!state.metaDesc && state.blocks.length > 0) {
      ...
      showToast("Ajoutez du contenu d'abord", false);
    }
  };
  ...
  const save = async (pub?: boolean) => {
    ...
    if (!state.company.trim()) {
      showToast("L'entreprise est requise", false);
      return;
    }
    if (state.skills.length === 0) {
      showToast("Ajoutez au moins une compétence", false);
      return;
    }
    setSaving(true);
    ...
    const { data, error } =
      existingId === null
        ? await (sb.from("opportunities") as any)
            .insert(payload)
            .select("id,slug")
            .single()
        : await (sb.from("opportunities") as any)
  ...
  const restoreDraft = () => {
    ...
    try {
      ...
      if (raw) {
        ...
        showToast("Brouillon local restauré");
      }
    } catch {
      showToast("Impossible de restaurer", false);
    }
  };
  const clearDraft = () => {
    if (autoSaveRef.current) clearTimeout(autoSaveRef.current);
  ...
  return (
    <div className="aa-editor-page">
      ...
      {toast && (
        <div
          className={`aa-toast ${toast.ok ? "aa-toast--ok" : "aa-toast--err"}`}
        >
          {toast.ok ? "✓" : "✕"} {toast.msg}
        </div>
      )}
      ...
      {pubModal !== null && (
        <div
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget) setPubModal(null);
          }}
        >
          <div className="aa-modal">
            ...
            <div className="aa-modal-title">
              {pubModal
                ? "Publier cette opportunité ?"
                : "Enregistrer les modifications"}
            </div>
            <p className="aa-modal-desc">
              {pubModal
                ? "L'opportunité sera visible par tous les visiteurs d'AroMe."
      ...
      <div className="aa-editor-topbar">
        ...
        <div className="aa-editor-indicators">
          {state.featured && (
            <span
              className="aa-editor-draft-badge"
              style={{ background: "rgba(192,132,53,.1)", color: "#C08435" }}
            >
              ⭐ En vedette
            </span>
          )}
          {state.remote && (
            <span
              className="aa-editor-draft-badge"
              style={{ background: "rgba(90,127,212,.1)", color: "#5A7FD4" }}
            >
              🌐 Remote
            </span>
          )}
          {hasDraft && !saving && (
            <span
              className="aa-editor-draft-badge"
              title="Brouillon local non sauvegardé"
            >
              ● local
            </span>
          )}
          <span
            className={`aa-editor-status-dot ${state.published ? "aa-editor-status-dot--pub" : "aa-editor-status-dot--draft"}`}
          />
          <span className="aa-editor-status-label">
      ...
      {hasDraft && itemIdRef.current && (
        <div className="aa-draft-banner">
          ...
          <button onClick={restoreDraft} className="aa-draft-banner-btn">
            Restaurer
          </button>
          <button onClick={clearDraft} className="aa-draft-banner-dismiss">
            Ignorer
          </button>
        </div>
      )}
            ...
            <div style={{ paddingTop: "1.5rem" }}>
              ...
              >
                <div className="op-field-group">
                  ...
                  <input
                    ...
                    placeholder="Ex : Google, Banque Mondiale, ONU…"
                  />
                </div>
              ...
              >
                <div className="op-field-group">
                  ...
                  <input
                    ...
                    placeholder="Ex : Nairobi, Abidjan, Remote…"
                  />
                </div>
                <div className="op-field-group">
                  ...
                  <input
                    ...
                    placeholder="Ex : Kenya, Côte d'Ivoire…"
                  />
                </div>
              </div>
              ...
              <div className="op-field-group">
                  ...
                  {COMMON_FLAGS.map((f) => (
                    <button
                      ...
                      style={{
                        ...
                        cursor: "pointer",
                        outline:
                          state.flag === f ? "2px solid #1A5C40" : "none",
                      }}
                    >
                      {f}
                    </button>
                ...
                <input
                  ...
                  placeholder="Emoji drapeau personnalisé"
                />
              </div>
              ...
              <div className="op-field-group">
                ...
                >
                  <input
                    type="text"
                    placeholder="Ou ajouter un nouveau secteur"
                    value={customSector}
                  ...
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: 10,
                      border: "none",
                      background: customSector.trim()
                        ? "#141410"
                        : "rgba(20,20,16,.1)",
                      color: customSector.trim() ? "#fff" : "#928E80",
                      cursor: customSector.trim() ? "pointer" : "default",
                      fontSize: "0.78rem",
                      fontWeight: 700,
                    }}
                  >
              ...
              >
                ...
                <div className="op-field-group">
                  ...
                  <input
                    ...
                    placeholder="Ex : 85 000 USD/an, Compétitif…"
                  />
                </div>
              </div>
              ...
              <div className="op-field-group">
                ...
                <input
                  ...
                  placeholder="https://careers.exemple.com/postuler"
                  type="url"
                />
              </div>
              ...
              <div className="op-field-group">
                ...
                <div className="aa-tag-row">
                  ...
                  />
                  <button className="aa-tag-add-btn" onClick={addSkill}>
                    Ajouter
                  </button>
                </div>
              </div>
          ...
          {tab === "content" && (
            <div style={{ paddingTop: "1.5rem" }}>
              <div className="op-field-group">
                ...
                >
                  Construisez le contenu de l'offre (description, avantages,
                  critères, processus de recrutement…) en ajoutant des blocs.
                </p>
                <BlockBuilder
                  blocks={state.blocks}
                ...
                <div style={{ marginTop: "2rem" }}>
                  ...
                  <div
                    style={{
                      ...
                      boxShadow: "0 2px 12px rgba(20,20,16,.07)",
                      maxWidth: 380,
                    }}
                  >
                    <div
                      style={{
                        height: 140,
                        position: "relative",
                        background: state.coverUrl
                          ? `url(${state.coverUrl}) center/cover`
                          : `linear-gradient(135deg,${tc}22,#0a0800)`,
                      }}
                    >
                      <div
                        style={{
                      ...
                      {!state.coverUrl && (
                        ...
                        >
                          ...
                          >
                            {state.companyInitials ||
                              state.company?.slice(0, 2).toUpperCase()}
                          </span>
                        </div>
                      )}
                    </div>
                    <div style={{ padding: "1.2rem 1.4rem" }}>
                      ...
                      <div style={{ fontSize: ".72rem", color: "#928E80" }}>
                        {state.flag} {state.company || "Entreprise"} ·{" "}
                      ...
                      {state.skills.length > 0 && (
                        ...
                        >
                          ...
                          {state.skills.length > 3 && (
                            <span
                              style={{ fontSize: ".55rem", color: "#928E80" }}
                            >
                              +{state.skills.length - 3}
                            </span>
                          )}
            ...
            <div className="aa-seo-panel" style={{ paddingTop: "1.5rem" }}>
              ...
              <div className="op-field-group">
                ...
                >
                  ...
                  <button
                    ...
                    style={{
                      fontSize: ".62rem",
                      fontWeight: 600,
                      color: state.slugLocked ? "#1A5C40" : "#928E80",
                      background: "none",
                      border: "none",
                      cursor: "pointer",
                      fontFamily: "inherit",
                ...
                <div className="aa-slug-wrap">
                  <span className="aa-slug-prefix">
                    afripulse.com/opportunites/
                  </span>
                  <input
                    className="aa-slug-input"
                    value={state.slug}
                    onChange={(e) => {
                      if (!state.slugLocked) set("slug", e.target.value);
                    }}
                  ...
                  >
                    <button
                      ...
                      style={{
                        ...
                        padding: ".2rem .6rem",
                        borderRadius: 100,
                        border: "1px solid rgba(26,92,64,.3)",
                        background: "rgba(26,92,64,.08)",
                        color: "#1A5C40",
                        cursor: state.title ? "pointer" : "default",
                        fontFamily: "inherit",
                      }}
                    >
                      🔄 Générer
                    ...
                    <span
                      className={`aa-char-count ${state.metaTitle.length > 60 ? "aa-char-count--err" : "aa-char-count--ok"}`}
                    >
              ...
              <div className="op-field-group">
                  ...
                  >
                    <button
                      ...
                      style={{
                        ...
                        background: "rgba(26,92,64,.08)",
                        color: "#1A5C40",
                        cursor: state.blocks.length > 0 ? "pointer" : "default",
                        fontFamily: "inherit",
                      }}
                    >
                      🔄 Générer
                    ...
                    <span
                      className={`aa-char-count ${state.metaDesc.length > 160 ? "aa-char-count--err" : "aa-char-count--ok"}`}
                    >
                ...
                {!state.metaDesc && state.blocks.length === 0 && (
                  ...
                  >
                    ⚠️ Ajoutez du contenu pour générer une description
                    automatique.
                  </div>
                )}
              </div>
              ...
              <div className="op-field-group">
                ...
                <div className="aa-seo-preview">
                  <div className="aa-seo-preview-url">
                    afripulse.com › opportunites › {previewSlug}
                  </div>
                  <div className="aa-seo-preview-title">{previewTitle}</div>
                  <div className="aa-seo-preview-desc">
                ...
                >
                  📌 Le titre SEO et la description sont générés automatiquement
                  si vous les laissez vides.
                </div>
              </div>
            </div>
          )}
        ...
        <aside className="aa-editor-sidebar">
          <div className="aa-sidebar-card">
            ...
            <div className="aa-sidebar-row">
              ...
              >
                <span
                  className={`aa-status-dot ${state.published ? "aa-status-dot--pub" : "aa-status-dot--draft"}`}
                />
                {state.published ? "Publiée" : "Brouillon"}
              </div>
            </div>
            <div className="aa-sidebar-row">
              ...
              >
                <span
                  className={`aa-toggle-knob ${state.featured ? "aa-toggle-knob--on" : "aa-toggle-knob--off"}`}
                  style={state.featured ? { background: "#C08435" } : {}}
                />
              </button>
            </div>
            <div className="aa-sidebar-row">
              ...
              <button
                className={`aa-toggle ${state.remote ? "aa-toggle--on" : "aa-toggle--off"}`}
                onClick={() => set("remote", !state.remote)}
              >
                <span
                  className={`aa-toggle-knob ${state.remote ? "aa-toggle-knob--on" : "aa-toggle-knob--off"}`}
                  style={state.remote ? { background: "#5A7FD4" } : {}}
                />
              </button>
            </div>
            ...
            {state.deadline && (
              ...
              >
                ...
                >
                  {new Date(state.deadline).toLocaleDateString("fr-FR", {
                    day: "numeric",
                    month: "long",
                    year: "numeric",
                  })}
