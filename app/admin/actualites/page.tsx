...
export default function AdminActualitesPage() {
  ...
  const togglePublished = async (id: string, current: boolean) => {
    const { error } = await (sb.from("articles") as any).update({ published: !current }).eq("id", id);
    if (!error) { showToast(current ? "Article dépublié" : "Article publié"); load(); loadStats(); }
  ...
  const toggleFeatured = async (id: string, current: boolean) => {
    ...
    if (!error) { showToast(current ? "Retiré de la une" : "Mis à la une"); load(); loadStats(); }
  ...
  const deleteArticles = async (ids: string[]) => {
    ...
    setConfirm(null);
    if (!error) { showToast(`${ids.length} article${ids.length > 1 ? "s" : ""} supprimé${ids.length > 1 ? "s" : ""}`); load(); loadStats(); }
    ...
    <div style={{ padding: "2.5rem 3rem", minHeight: "100vh", background: "var(--bg2)" }}>
      ...
      {toast && (
        <div style={{
          ...
          fontSize: ".82rem", fontWeight: 600, boxShadow: "0 8px 32px rgba(20,20,16,.25)",
      ...
      {/* ── Confirm dialog ── */}
      {confirm && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(20,20,16,.55)",
          backdropFilter: "blur(4px)", zIndex: 9000, display: "flex",
          alignItems: "center", justifyContent: "center" }}>
          <div style={{ background: "#fff", borderRadius: 20, padding: "2rem 2.5rem",
            maxWidth: 440, width: "100%", margin: "0 1rem", boxShadow: "0 24px 64px rgba(20,20,16,.18)" }}>
            <div style={{ fontFamily: "'Fraunces',Georgia,serif", fontSize: "1.25rem",
              fontWeight: 900, color: "#141410", marginBottom: ".75rem" }}>
              {confirm.type === "delete" || confirm.type === "bulk-delete"
                ? "Supprimer ?"
                : confirm.type === "bulk-toggle" ? "Modifier le statut ?" : ""}
            </div>
            ...
            <div style={{ display: "flex", gap: ".75rem", justifyContent: "flex-end" }}>
              <button onClick={() => setConfirm(null)}
                style={{ padding: ".6rem 1.2rem", borderRadius: 100, border: "1.5px solid rgba(20,20,16,.15)",
                  background: "transparent", cursor: "pointer", fontFamily: "inherit",
                  fontSize: ".82rem", fontWeight: 600, color: "#38382E" }}>
                Annuler
              </button>
              <button
                onClick={() => {
                ...
                style={{ padding: ".6rem 1.4rem", borderRadius: 100, border: "none",
                  background: "#B8341E", color: "#fff", cursor: "pointer",
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between",
        ...
        <div>
          <div style={{ fontSize: ".58rem", fontWeight: 700, letterSpacing: ".2em",
            textTransform: "uppercase", color: "#C08435", marginBottom: ".4rem" }}>
            AfriPulse Admin
          </div>
          <h1 style={{ fontFamily: "'Fraunces',Georgia,serif", fontSize: "2rem",
            fontWeight: 900, letterSpacing: "-.04em", color: "#141410", margin: 0 }}>
            Actualités
          </h1>
        ...
        <div style={{ display: "flex", gap: ".6rem", flexWrap: "wrap" }}>
          <button onClick={exportCSV}
            style={{ display: "inline-flex", alignItems: "center", gap: ".4rem",
              ...
              border: "1.5px solid rgba(20,20,16,.15)", transition: "all .18s" }}>
          ...
          <Link href="/admin/actualites/nouveau"
            style={{ display: "inline-flex", alignItems: "center", gap: ".45rem",
              ...
              color: "#fff", textDecoration: "none", transition: "opacity .18s" }}>
        ...
        {[
          ...
          <div key={i} style={{ background: "#fff", borderRadius: 16, padding: "1.1rem 1.3rem",
            ...
            <div style={{ fontSize: ".58rem", fontWeight: 700, letterSpacing: ".14em",
              textTransform: "uppercase", color: "#928E80", marginBottom: ".3rem" }}>
              {s.label}
            </div>
            <div style={{ fontFamily: "'Fraunces',Georgia,serif", fontSize: "1.6rem",
              fontWeight: 900, color: s.color, letterSpacing: "-.04em", lineHeight: 1 }}>
              {fmtNum(s.value)}
            </div>
      ...
      <div style={{ background: "#fff", borderRadius: 16, border: "1px solid var(--border)",
        padding: "1.1rem 1.4rem", marginBottom: "1.25rem", boxShadow: "var(--s1)",
        display: "flex", gap: "1rem", flexWrap: "wrap", alignItems: "center" }}>

        {/* Recherche */}
        <div style={{ display: "flex", alignItems: "center", gap: ".5rem",
          background: "#F8F6F1", border: "1.5px solid rgba(20,20,16,.09)",
          borderRadius: 100, padding: ".42rem .42rem .42rem 1rem", flex: "0 0 260px" }}>
          ...
          {searchDraft && (
            <button onClick={() => { setSearchDraft(""); setSearch(""); setPage(1); }}
              style={{ background: "rgba(20,20,16,.08)", border: "none", borderRadius: "50%",
                width: 20, height: 20, cursor: "pointer", fontSize: ".65rem",
                color: "#928E80", display: "flex", alignItems: "center", justifyContent: "center",
                flexShrink: 0 }}>✕</button>
          )}
        </div>
        ...
        <div style={{ display: "flex", gap: ".35rem" }}>
          {([
            ...
            <button key={f.key} onClick={() => { setStatus(f.key); setPage(1); }}
              style={{ fontFamily: "'DM Sans',system-ui,sans-serif", fontSize: ".72rem",
                fontWeight: 600, padding: ".38rem .85rem", borderRadius: 100, cursor: "pointer",
                transition: "all .15s",
                background: status === f.key ? "#141410" : "transparent",
                color:      status === f.key ? "#fff"    : "#38382E",
                border:     status === f.key ? "none"    : "1.5px solid rgba(20,20,16,.12)" }}>
              {f.label}
            </button>
          ))}
        </div>
        ...
        <select
          ...
          style={{ fontFamily: "'DM Sans',system-ui,sans-serif", fontSize: ".78rem",
            fontWeight: 500, padding: ".42rem .9rem", borderRadius: 100,
            border: "1.5px solid rgba(20,20,16,.12)", background: "#fff",
            color: category ? "#141410" : "#928E80", cursor: "pointer",
      ...
      {selected.size > 0 && (
        <div style={{ background: "#141410", borderRadius: 12, padding: ".85rem 1.4rem",
          ...
          boxShadow: "0 4px 16px rgba(20,20,16,.2)", animation: "slideUp .2s ease" }}>
          ...
          <button onClick={() => bulkPublish(Array.from(selected), true)}
            style={{ fontFamily: "inherit", fontSize: ".75rem", fontWeight: 600,
              color: "#4A9E6F", background: "rgba(74,158,111,.12)", border: "none",
              padding: ".38rem .85rem", borderRadius: 100, cursor: "pointer" }}>
            Publier tout
          </button>
          <button onClick={() => bulkPublish(Array.from(selected), false)}
            style={{ fontFamily: "inherit", fontSize: ".75rem", fontWeight: 600,
              color: "#C08435", background: "rgba(192,132,53,.12)", border: "none",
              padding: ".38rem .85rem", borderRadius: 100, cursor: "pointer" }}>
            Dépublier tout
          </button>
          <button onClick={() => setConfirm({ type: "bulk-delete", ids: Array.from(selected) })}
            style={{ fontFamily: "inherit", fontSize: ".75rem", fontWeight: 600,
              color: "#C25B3F", background: "rgba(194,91,63,.12)", border: "none",
              padding: ".38rem .85rem", borderRadius: 100, cursor: "pointer" }}>
            Supprimer
          </button>
          <button onClick={() => setSelected(new Set())}
            style={{ marginLeft: "auto", fontFamily: "inherit", fontSize: ".72rem",
      ...
      <div style={{ background: "#fff", borderRadius: 20, border: "1px solid var(--border)",
        boxShadow: "var(--s2)", overflow: "hidden" }}>

        {/* En-tête colonnes */}
        <div style={{ display: "grid",
          gridTemplateColumns: "40px 3fr 110px 110px 90px 90px 90px 110px",
          gap: "0 .75rem", padding: ".75rem 1.4rem",
          background: "#F8F6F1", borderBottom: "1px solid var(--border)" }}>
          {/* Checkbox all */}
        ...
        {loading ? (
          ...
          <div style={{ padding: "5rem", textAlign: "center" }}>
            <div style={{ fontFamily: "'Fraunces',Georgia,serif", fontSize: "1.2rem",
              fontWeight: 700, color: "#141410", marginBottom: ".5rem" }}>
              Aucun article
            </div>
          ...
          return (
            <div key={art.id}
              style={{ display: "grid",
                gridTemplateColumns: "40px 3fr 110px 110px 90px 90px 90px 110px",
                gap: "0 .75rem", padding: ".9rem 1.4rem", alignItems: "center",
                borderBottom: i < articles.length - 1 ? "1px solid rgba(20,20,16,.05)" : "none",
                background: isSel ? "rgba(192,132,53,.03)" : "transparent",
                transition: "background .12s" }}
              onMouseEnter={e => { if (!isSel) (e.currentTarget as HTMLDivElement).style.background = "#FAFAF8"; }}
              onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.background = isSel ? "rgba(192,132,53,.03)" : "transparent"; }}>

              {/* Checkbox */}
              <input type="checkbox" checked={isSel} onChange={() => toggleSelect(art.id)}
                style={{ width: 15, height: 15, cursor: "pointer", accentColor: "#C08435" }}
              />
              ...
              <div style={{ display: "flex", alignItems: "center", gap: ".85rem", minWidth: 0 }}>
                ...
                <div style={{ minWidth: 0 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: ".5rem" }}>
                    {art.featured && (
                      <span style={{ fontSize: ".52rem", fontWeight: 800, letterSpacing: ".08em",
                        textTransform: "uppercase", color: "#C08435",
                        background: "rgba(192,132,53,.1)", padding: ".12rem .45rem", borderRadius: 100 }}>
                        Une
                      </span>
                    )}
              ...
              <span style={{ display: "inline-block", fontSize: ".56rem", fontWeight: 800,
                ...
                color: cc, background: `${cc}14`, padding: ".22rem .65rem", borderRadius: 100 }}>
                {art.category}
              </span>
              ...
              {/* Vues */}
              <span style={{ fontFamily: "'Fraunces',Georgia,serif", fontSize: ".88rem",
                fontWeight: 700, color: "#141410", letterSpacing: "-.02em" }}>
                {fmtNum(art.views)}
              </span>

              {/* Saves */}
              <span style={{ fontFamily: "'Fraunces',Georgia,serif", fontSize: ".88rem",
                fontWeight: 700, color: "#5A7FD4", letterSpacing: "-.02em" }}>
              ...
              <button
                ...
                style={{ display: "inline-flex", alignItems: "center", gap: ".35rem",
                  fontFamily: "inherit", fontSize: ".62rem", fontWeight: 700,
                  letterSpacing: ".06em", textTransform: "uppercase",
                  padding: ".28rem .7rem", borderRadius: 100, cursor: "pointer", border: "none",
                  background:  art.published ? "rgba(26,92,64,.1)"  : "rgba(20,20,16,.06)",
              ...
              <div style={{ display: "flex", alignItems: "center", gap: ".5rem", justifyContent: "flex-end" }}>
                ...
                {/* Actions */}
                <div style={{ display: "flex", gap: ".25rem", opacity: 0 }}
                  className="row-actions">
                  <button
                    title="Vedette"
                    onClick={() => toggleFeatured(art.id, art.featured)}
                    style={{ width: 26, height: 26, borderRadius: 8, border: "none",
                      background: art.featured ? "rgba(192,132,53,.12)" : "rgba(20,20,16,.06)",
                      color: art.featured ? "#C08435" : "#928E80",
                      cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <IcoStar filled={art.featured} />
                  </button>
                  <Link href={`/actualites/${art.slug}`} target="_blank"
                    title="Voir sur le site"
                    style={{ width: 26, height: 26, borderRadius: 8, border: "none",
                      background: "rgba(20,20,16,.06)", color: "#928E80",
                      cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center",
                      textDecoration: "none" }}>
                    <IcoEye />
                  </Link>
                  <Link href={`/admin/actualites/${art.id}`}
                    title="Éditer"
                    style={{ width: 26, height: 26, borderRadius: 8, border: "none",
                      background: "rgba(30,77,168,.08)", color: "#1E4DA8",
                      cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center",
                      textDecoration: "none" }}>
                    <IcoEdit />
                  </Link>
                  <button
                    title="Supprimer"
                    onClick={() => setConfirm({ type: "delete", ids: [art.id] })}
                    style={{ width: 26, height: 26, borderRadius: 8, border: "none",
          ...
          <div style={{ display: "flex", gap: ".4rem" }}>
            <button onClick={() => { setPage(p => Math.max(1,p-1)); }}
              ...
              style={{ display:"flex", alignItems:"center", gap:".3rem",
                ...
                padding:".45rem .9rem", borderRadius:100, cursor: page===1?"not-allowed":"pointer",
                background:"transparent", color: page===1?"#D0CCBF":"#38382E",
                border:"1.5px solid", borderColor: page===1?"#E8E4DC":"rgba(20,20,16,.15)" }}>
              ...
              return (
                <button key={p} onClick={() => setPage(p)}
                  style={{ width:34,height:34, borderRadius:"50%", cursor:"pointer",
                    ...
                    border:     page===p?"none":"1.5px solid rgba(20,20,16,.12)",
                    transition:"all .15s" }}>
                  {p}
                </button>
              );
            ...
            <button onClick={() => { setPage(p => Math.min(totalPages,p+1)); }}
              ...
              style={{ display:"flex", alignItems:"center", gap:".3rem",
                ...
                background:"transparent", color: page===totalPages?"#D0CCBF":"#38382E",
                border:"1.5px solid", borderColor: page===totalPages?"#E8E4DC":"rgba(20,20,16,.15)" }}>
