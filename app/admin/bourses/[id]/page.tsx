...
/* ═══════════════════════════════════════════════════════════
   PAGE
═══════════════════════════════════════════════════════════ */
export default function BourseEditorPage({
  params,
}: {
  params: { id: string };
}) {
  ...
  const set = useCallback(
    <K extends keyof BourseState>(k: K, v: BourseState[K]) =>
  ...
  useEffect(() => {
    sb.from("scholarships")
      ...
      .then(({ data }) => {
        if (data) {
          const unique = [
            ...new Set(data.map((d: any) => d.domain).filter(Boolean)),
          ];
    ...
    autoSaveRef.current = setTimeout(() => {
      ...
      try {
        localStorage.setItem(
          `afripulse_draft_scholarship_${itemIdRef.current}`,
  ...
  const getDefaultMetaTitle = () => {
    if (state.title) {
      return `${state.title} — Bourse d'études | AroMe`;
    }
    return "Bourse d'études | AroMe";
  };
  ...
  const fillDefaultMetaDesc = () => {
    if (!state.metaDesc && state.blocks.length > 0) {
      ...
      showToast(
        "Ajoutez du contenu d'abord pour générer une description",
        false,
      );
    }
  };
  ...
  const save = async (pub?: boolean) => {
    ...
    if (!state.organization.trim()) {
      showToast("L'organisme est requis", false);
      return;
    }
    if (!state.deadline) {
      showToast("La date limite est requise", false);
      return;
    }
    if (state.tags.length === 0) {
      showToast("Ajoutez au moins un tag", false);
      return;
    }
    if (!state.domain.trim()) {
      showToast("Le domaine est requis", false);
      return;
    }
    setSaving(true);
    ...
    const { data, error } =
      existingId === null
        ? await (sb.from("scholarships") as any)
            .insert(payload)
            .select("id,slug")
            .single()
        : await (sb.from("scholarships") as any)
        ...
        >
          <div className="aa-modal">
            ...
            <div className="aa-modal-title">
              {pubModal
                ? "Publier cette bourse ?"
                : "Enregistrer les modifications"}
            </div>
            <p className="aa-modal-desc">
              {pubModal
                ? "La bourse sera visible par tous les visiteurs d'AroMe."
        ...
        <div className="aa-editor-indicators">
          {state.urgent && (
            <span
              ...
              style={{ background: "rgba(184,52,30,.1)", color: "#B8341E" }}
            >
              ⚡ Urgent
            </span>
          )}
            ...
            <div style={{ paddingTop: "1.5rem" }}>
                    ...
                    {COMMON_FLAGS.map((f) => (
                      <button
                        ...
                        style={{
                          ...
                          cursor: "pointer",
                          outline:
                            state.flag === f ? "2px solid #C08435" : "none",
                        }}
                      >
                        {f}
                      </button>
              ...
              <div className="bs-field-group">
                ...
                <div className="aa-tag-row">
                  ...
                  />
                  <button className="aa-tag-add-btn" onClick={addTag}>
                    Ajouter
                  </button>
                </div>
              </div>
          ...
          {tab === "content" && (
            <div style={{ paddingTop: "1.5rem" }}>
              <div className="bs-field-group">
                ...
                >
                  Construisez le contenu de votre bourse (description,
                  conditions, avantages, etc.) en ajoutant des blocs.
                </p>
                <BlockBuilder
                  blocks={state.blocks}
                  ...
                  >
                    <div
                      style={{
                        ...
                        background: state.coverUrl
                          ...
                          : `linear-gradient(135deg,${lvlColor}22,#0a0800)`,
                      }}
                    >
                      <div
                        style={{
                    ...
                    <div style={{ padding: "1.2rem 1.4rem" }}>
                      ...
                      <div style={{ fontSize: ".72rem", color: "#928E80" }}>
                        {state.flag} {state.organization || "Organisme"} ·{" "}
                        {state.deadline
                          ? new Date(state.deadline).toLocaleDateString(
                              "fr-FR",
              ...
              <div className="bs-field-group">
                ...
                <div className="aa-seo-preview">
                  <div className="aa-seo-preview-url">
                    afripulse.com › bourses › {previewSlug}
                  </div>
