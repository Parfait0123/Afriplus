...
/**
 * app/admin/page.tsx — Vue d'ensemble AroMe Admin
...
const SvgCal = ({ c }: { c: string }) => (
  ...
  >
    ...
    <circle cx="8" cy="15" r="1.5" fill={c} stroke="none" />
    <circle cx="12" cy="15" r="1.5" fill={c} stroke="none" />
  </svg>
);
const SvgMail = ({ c }: { c: string }) => (
  <svg
    width="20"
    height="20"
    viewBox="0 0 24 24"
  ...
  return (
    ...
    >
      ...
      >
        ...
        {trend !== undefined && (
          <span
            className={`adm-badge ${pos ? "adm-badge--green" : "adm-badge--red"}`}
          >
            {pos ? "+" : ""}
            {trend}%
          </span>
...
function Bar({
  ...
  return (
    <div style={{ marginBottom: "1.1rem" }}>
      ...
      >
        <span
          style={{ fontSize: "0.78rem", fontWeight: 600, color: "#38382E" }}
        >
          {label}
        </span>
...
function SH({
  ...
  return (
    <div className="adm-section-header">
      ...
      {action && href && (
        <Link href={href} className="adm-section-link">
          {action}
        </Link>
      )}
    </div>
...
function BarChart({ color }: { color: string }) {
  const bars = [
    0.42, 0.65, 0.52, 0.78, 0.6, 0.88, 0.72, 0.84, 0.68, 1.0, 0.79, 0.92, 0.75,
    0.95,
  ];
  return (
...
export default function AdminDashboard() {
  ...
  const sp = (n: number) =>
    Array.from({ length: 14 }, (_, i) =>
      Math.max(
        0,
        Math.round(n * (0.55 + (i / 13) * 0.45) * (0.8 + Math.random() * 0.4)),
      ),
    );
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [aR, bR, oR, eR, subR, usrR, savR, appR] = await Promise.all([
        ...
        supabase
          .from("scholarships")
          .select("id,title,slug,organization,deadline,urgent,level", {
            count: "exact",
          })
          .order("created_at", { ascending: false })
          .limit(6),
        ...
        supabase
          ...
          .select("id,confirmed", { count: "exact" }),
        supabase
          .from("profiles")
          .select("id,full_name,email,role,created_at,country")
          .order("created_at", { ascending: false })
          .limit(6),
    ...
    <div className="adm-page">
        ...
        <div className="adm-header-inner">
          <div>
            ...
            <p className="adm-greeting-sub">
              Vue d&apos;ensemble en temps réel · {formatNumber(total)} contenus
              publiés
            </p>
          </div>
      ...
      <div className="adm-body">
        ...
        <div className="adm-week-banner">
          <div>
            ...
            <div style={{ display: "flex", gap: "2rem", flexWrap: "wrap" }}>
              {[
                ...
                {
                  label: "Candidatures",
                  v: stats?.applications ?? 0,
                  color: "#1A5C40",
                },
                {
                  label: "Abonnés conf.",
                  v: stats?.abonnesConfirmed ?? 0,
                  color: "#1E4DA8",
                },
              ].map(({ label, v, color }) => (
                <div key={label}>
          ...
          <div
            style={{
              borderLeft: "1px solid rgba(192,132,53,.2)",
              paddingLeft: "1.5rem",
            }}
          >
        ...
        <div className="adm-kpi-grid">
          <KpiCard
            ...
            value={stats?.actualitesPublished ?? 0}
            sub={`${stats?.actualites ?? 0} au total`}
            color="#C08435"
            href="/admin/articles"
            Icon={SvgNews}
            spark={sp(stats?.actualitesPublished ?? 6)}
            trend={8}
            delay={0}
          />
          <KpiCard
            ...
            sub={`${stats?.boursesUrgent ?? 0} urgentes`}
            color="#1A5C40"
            href="/admin/bourses"
            Icon={SvgCap}
            spark={sp(stats?.bourses ?? 5)}
            trend={12}
            delay={60}
          />
          <KpiCard
          ...
          <KpiCard
            ...
            sub={`${stats?.abonnesConfirmed ?? 0} confirmés`}
            color="#2D6B3B"
            href="/admin/abonnes"
            Icon={SvgMail}
            spark={sp(stats?.abonnes ?? 10)}
            trend={22}
            delay={240}
          />
          <KpiCard
          ...
          <KpiCard
            ...
            value={engagement}
            sub="saves + candidatures"
            color="#1A5C5C"
            href="/admin/analytique"
            Icon={SvgHeart}
            spark={sp(engagement ?? 4)}
            trend={9}
            delay={360}
          />
        </div>
        ...
        <div className="adm-main-grid">
          ...
          <div className="adm-card" style={{ padding: 0, overflow: "hidden" }}>
              ...
              <div className="adm-tabs">
                {(["actus", "bourses"] as const).map((k) => (
                  <button
                    key={k}
                    onClick={() => setTab(k)}
                    className={`adm-tab-btn ${tab === k ? "adm-tab-btn--active" : ""}`}
                  >
            ...
            <div style={{ padding: "0.25rem 1.5rem 1rem" }}>
              {tab === "actus" ? (
                actus.length === 0 ? (
                  <div className="adm-empty">Aucune actualité.</div>
                ) : (
                  actus.map((a, i) => {
                    ...
                    return (
                      <div
                        ...
                        style={{
                          borderTop:
                            i === 0 ? "none" : "1px solid rgba(20,20,16,.05)",
                        }}
                      >
                        <span className="adm-list-num">
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          >
                            ...
                            >
                              {formatDate(a.created_at, { relative: true })}
                            </span>
                          </div>
                        </div>
                        <div
                        ...
                        >
                          <span
                            className={`adm-badge ${a.published ? "adm-badge--green" : "adm-badge--muted"}`}
                          >
                ...
                <div className="adm-empty">Aucune bourse.</div>
              ) : (
                bourses.map((b, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <span
                          className={`adm-badge ${b.urgent ? "adm-badge--red" : "adm-badge--green"}`}
                        >
                          {b.urgent ? "Urgent" : "Ouvert"}
                        </span>
                        <Link
                          href={`/admin/bourses/${b.id}`}
                          className="adm-icon-btn"
                          title="Éditer"
                        >
                          <IcoEdit />
          ...
          >
            ...
            <div className="adm-card adm-card--dark">
              ...
              <div className="adm-dark-value">
                <CountUp target={engagement} />
              </div>
              <div className="adm-dark-label">
                saves et candidatures suivies par les membres
              </div>
                ...
                {[
                  ...
                  <div
                    ...
                    style={{
                      ...
                      padding: "0.5rem 0",
                      borderLeft:
                        i > 0 ? "1px solid rgba(255,255,255,.06)" : "none",
                    }}
                  >
                    <div
                      style={{
        ...
        <div className="adm-secondary-grid">
          ...
          <div className="adm-card">
            ...
            />
            {users.length === 0 ? (
              <div className="adm-empty">Aucun utilisateur.</div>
            ) : (
              users.map((u, i) => {
                const rs = ROLE[u.role] ?? ROLE.reader;
                const ini = u.full_name
                  ? u.full_name
                      .split(" ")
                      .map((w) => w[0])
                      .join("")
                ...
                return (
                  ...
                  >
                    <div
                      className="adm-avatar-sm"
                      style={{
                        background: `linear-gradient(135deg, ${rs.color}99, ${rs.color}55)`,
                      }}
                    >
                      {ini}
                    </div>
                    ...
                    >
                      ...
                      <span style={{ fontSize: "0.58rem", color: "#928E80" }}>
          ...
          <div className="adm-card">
            ...
            >
              {[
                ...
                {
                  label: "Ajouter une bourse",
                  href: "/admin/bourses/nouveau",
                  icon: "🎓",
                  color: "#1A5C40",
                  bg: "#EAF4EF",
                },
                {
                  label: "Poster une opportunité",
            ...
            <div style={{ display: "flex", gap: "0.5rem", marginTop: "1rem" }}>
              {[
                { label: "Analytique", href: "/admin/analytique" },
          ...
          <div className="adm-card">
            ...
            >
              <span style={{ fontSize: "0.72rem", color: "#928E80" }}>
                Total contenus
              </span>
              <span
                style={{
