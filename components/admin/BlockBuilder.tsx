    ...
    items: [
      {
        ...
        icon: "🖼",
        label: "Image",
        desc: "Uploader depuis le PC ou coller un lien",
        color: "#1A5C5C",
      },
      {
        type: "video" as const,
...
function emptyBlock(type: Block["type"]): Block {
  switch (type) {
    ...
    case "pullquote":
      return { type, text: "", author: "", role: "" };
    case "image":
      return { type, url: "", alt: "", caption: "" };
    case "video":
      return { type, url: "", caption: "" };
    case "factbox":
      return { type, title: "Chiffres clés", facts: [""] };
    case "checklist":
      return { type, title: "", items: [{ label: "", detail: "" }] };
    case "steps":
      return { type, title: "", items: [{ label: "", desc: "" }] };
    case "benefits":
      return { type, items: [{ icon: "✅", label: "", value: "" }] };
    case "compare":
      return {
        type,
        title: "",
    ...
    case "alert":
      return { type, message: "", variant: "info" };
    case "apply":
      return {
        type,
    ...
    case "external":
      return { type, url: "", label: "", description: "" };
    case "download":
      return { type, url: "", label: "Télécharger le document", size: "" };
    case "related":
    ...
    case "agenda":
      return {
        type,
        title: "Programme",
        sessions: [{ time: "09h00", title: "", speaker: "" }],
      };
    case "speakers":
      return {
        type,
        title: "Intervenants",
        people: [{ name: "", role: "", org: "" }],
      };
    case "location":
      return { type, label: "", address: "", mapUrl: "" };
    case "divider":
      return { type };
    default:
...
const addRowBtn: React.CSSProperties = {
  ...
  borderRadius: 8,
  border: "1.5px dashed rgba(20,20,16,.15)",
...
function ImageBlockEditor({
  ...
  const patch = (p: Partial<typeof block>) =>
    onChange({ ...block, ...p } as Block);

  const handleFile = async (file: File) => {
    if (!file.type.startsWith("image/")) {
      setErrMsg("Format non supporté. Utilisez JPG, PNG ou WebP.");
      return;
    }
    if (file.size > 8 * 1024 * 1024) {
      setErrMsg("Image trop lourde (max 8 Mo).");
      return;
    }
    setErrMsg(null);
    setUploading(true);
        ...
        <>
          ...
          {mode === "upload" && (
            <div
              ...
              style={{
                border: "2px dashed rgba(20,20,16,.15)",
                borderRadius: 12,
...
function BlockEditor({
  ...
  const set = useCallback(
    (patch: Partial<Block>) => onChange({ ...block, ...patch } as Block),
    [block, onChange],
  );
  ...
  switch (block.type) {
      ...
      return (
        ...
        >
          <div style={field}>
            ...
            >
              💡 Sur YouTube : clic droit → &quot;Copier le code
              d&apos;intégration&quot; → copier l&apos;URL du src
            </div>
          </div>
          <div style={field}>
          ...
          {block.facts.map((f, i) => (
            <div key={i} style={{ display: "flex", gap: "0.5rem" }}>
              ...
              <button
                style={removeBtn}
                onClick={() =>
                  set({ facts: block.facts.filter((_, j) => j !== i) })
                }
              >
                ✕
              </button>
      ...
      return (
        ...
        >
            ...
            >
              <div style={{ display: "flex", gap: "0.5rem" }}>
                ...
                <button
                  style={removeBtn}
                  onClick={() =>
                    set({ items: block.items.filter((_, j) => j !== i) })
                  }
                >
                  ✕
                </button>
          ...
          <button
            style={addRowBtn}
            onClick={() =>
              set({ items: [...block.items, { label: "", detail: "" }] })
            }
          >
            + Ajouter un point
          </button>
        ...
        >
          ...
          {block.items.map((step, i) => (
            ...
            >
              ...
              <button
                style={{ ...removeBtn, alignSelf: "flex-start" }}
                onClick={() =>
                  set({ items: block.items.filter((_, j) => j !== i) })
                }
              >
          ...
          <button
            style={addRowBtn}
            onClick={() =>
              set({ items: [...block.items, { label: "", desc: "" }] })
            }
          >
            + Ajouter une étape
          </button>
      ...
      return (
        ...
        >
          ...
          {block.items.map((b, i) => (
            ...
            >
              ...
              <input
                ...
                style={inp}
                placeholder="Ex : 100% des frais couverts"
              />
              <button
                style={removeBtn}
                onClick={() =>
          ...
          <button
            ...
            onClick={() =>
              set({
                items: [...block.items, { icon: "✅", label: "", value: "" }],
              })
            }
          >
            + Ajouter un avantage
            ...
            <button
              ...
              onClick={() =>
                set({
                  columns: [
                    ...block.columns,
                    { label: `Colonne ${block.columns.length + 1}` },
                  ],
                  rows: block.rows.map((r) => ({
                    ...r,
                    values: [...r.values, ""],
            ...
            >
              ...
              {row.values.map((val, vi) => (
                <input
                  ...
                  style={inp}
                  placeholder={block.columns[vi]?.label || `Val ${vi + 1}`}
                />
              ))}
              <button
                style={removeBtn}
                onClick={() =>
                  set({ rows: block.rows.filter((_, j) => j !== ri) })
                }
              >
                ✕
              </button>
        ...
        >
          <div style={field}>
            ...
            <select
              ...
              onChange={(e) =>
                set({ variant: e.target.value as "info" | "warning" | "tip" })
              }
              style={inp}
            >
          ...
          >
            <div style={field}>
              ...
              <input
                ...
                onChange={(e) => set({ label: e.target.value })}
                style={inp}
                placeholder="Ex : Rapport AroMe 2026"
              />
            </div>
            <div style={field}>
        ...
        >
          ...
          <div style={field}>
            ...
            <input
              ...
              onChange={(e) => set({ label: e.target.value })}
              style={inp}
              placeholder="Laissez vide pour afficher le titre automatiquement"
            />
          </div>
        </div>
      );
          ...
          {block.sessions.map((s, i) => (
            ...
            >
              ...
              <button
                style={removeBtn}
                onClick={() =>
                  set({ sessions: block.sessions.filter((_, j) => j !== i) })
                }
              >
                ✕
              </button>
      ...
      return (
        ...
        >
          ...
          {block.people.map((p, i) => (
            ...
            >
              ...
              <button
                style={removeBtn}
                onClick={() =>
                  set({ people: block.people.filter((_, j) => j !== i) })
                }
              >
                ✕
              </button>
          ...
          <button
            ...
            onClick={() =>
              set({
                people: [...block.people, { name: "", role: "", org: "" }],
              })
            }
          >
    ...
    case "location":
      ...
      const handleIframePaste = () => {
        ...
        if (url) {
          ...
          setExtractError(
            "Impossible d'extraire le lien. Collez l'intégralité du code iframe Google Maps.",
          );
        }
      };
    ...
    default:
      return (
        <div style={{ color: "#928E80", fontSize: "0.82rem" }}>
          Type inconnu
        </div>
      );
  }
...
export default function BlockBuilder({ blocks, onChange }: BlockBuilderProps) {
  ...
  const getDef = (type: Block["type"]) => {
    for (const g of BLOCK_DEFS) {
      const f = g.items.find((d) => d.type === type);
      if (f) return f;
    }
    return null;
  };
  ...
  const blockPreview = (b: Block): string => {
    switch (b.type) {
      case "paragraph":
        return b.text.slice(0, 60) || "Paragraphe vide…";
      case "heading":
        return b.text || "Titre vide…";
      case "pullquote":
        return b.text ? `❝ ${b.text.slice(0, 50)}` : "Citation vide…";
      case "image":
        return b.url
          ? "🖼 " + (b.url.split("/").pop() ?? b.url.slice(0, 40))
          : "Aucune image";
      case "video":
        return b.url ? "▶ " + b.url.slice(0, 40) : "Aucune vidéo";
      case "factbox":
        return `${b.facts.filter(Boolean).length} élément(s) · ${b.title}`;
      case "checklist":
        return `${b.items.length} point(s)${b.title ? " · " + b.title : ""}`;
      case "steps":
        return `${b.items.length} étape(s)${b.title ? " · " + b.title : ""}`;
      case "benefits":
        return `${b.items.length} avantage(s)`;
      case "compare":
        return `${b.rows.length} ligne(s) · ${b.columns.length} colonne(s)`;
      case "alert":
        return b.message.slice(0, 60) || "Message vide…";
      case "apply":
        return b.label || "CTA vide";
      case "external":
        return b.label || b.url.slice(0, 50) || "Lien vide…";
      case "download":
        return b.label || "Fichier vide";
      case "related":
        return b.slug || "Slug manquant";
      case "agenda":
        return `${b.sessions.length} session(s)`;
      case "speakers":
        return `${b.people.length} personne(s)`;
      case "location":
        return b.label || "Lieu vide";
      case "divider":
        return "─────────────────";
      default:
        return "";
      ...
      {blocks.length === 0 ? (
        <div
          style={{
            background: "#fff",
            borderRadius: 16,
            border: "2px dashed rgba(20,20,16,.1)",
            padding: "2.5rem",
            textAlign: "center",
            marginBottom: "0.75rem",
          }}
        >
          ...
          <div style={{ fontSize: "0.8rem", color: "#928E80" }}>
            Cliquez sur &quot;+ Ajouter un bloc&quot; pour construire votre
            contenu
          </div>
        </div>
      ) : (
      ...
      {showPicker && (
        <div
          style={{
            ...
            padding: "1.25rem",
            marginTop: "0.5rem",
            boxShadow: "0 8px 32px rgba(20,20,16,.1)",
          }}
        >
          <div
            style={{
